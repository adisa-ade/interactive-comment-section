...
function CardContainer() {
    const { comments: replies, handleScoreChange } = useCommentScores(repliesData);    
    return (
        <div>
            {replies.map((reply,index) => (
            <div key={index} className="bg-(--white) m-6 rounded-[10px] p-6 space-y-4 lg:w-212">                                      
           ...
           <div className="flex items-center gap-6 bg-(--grey-50) py-2 px-4 rounded-[5px] lg:flex-col">            
              ...
              <Button onclick={() => handleScoreChange(index, -1)}>-</Button>
                  ...
                  <div className='flex items-center space-x-4'>
                      <img src={reply.user.image.png} alt={reply.user.username} className="w-14 h-14 rounded-full" />
                      <h1 className='text-(--grey-800) font-bold'>{reply.user.username}</h1>
                      <p className='text-(--grey-500) font-light'>{reply.createdAt}</p>
                  </div>
                  <div>
                      <p className='text-(--grey-500) text-[1.2rem] font-light'>{reply.content}</p>
                  </div>
                  </div>                
                  </div>
